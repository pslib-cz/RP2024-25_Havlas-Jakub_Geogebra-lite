...
export function validateAndCleanExpression(
...
): { valid: boolean; message: string; cleaned?: string; variable?: string } {
  ...
  // Check for optional assignment like `a=3`
  let variableMatch = expr.match(/^([a-zA-Z]+)=/);
  let assignedVariable: string | undefined;
  if (variableMatch) {
    assignedVariable = variableMatch[1];
    expr = expr.slice(variableMatch[0].length); // remove the assignment part
  }
  ...
  expr = expr.replace(/(\d)([a-zA-Z(])/g, "$1*$2");       // 2x, 3sin(x) → 2*x
  expr = expr.replace(/([a-zA-Z])(\d|\()/g, "$1*$2");     // x2, x( → x*2
  ...
  for (const token of matches) {
    ...
    if (/^[a-zA-Z]+$/.test(token)) continue;  // allow variables like a, b, myVar
