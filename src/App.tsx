...
function App() {
  return (
    <>
      <LibraryController
        expressions={["x^2", "sin(x)", "cos(x)", "tan(x)", "log(x)", "sqrt(x)"]}
...
let LibraryController = ({
  ...
  const functionsFromStorage = JSON.parse(
    localStorage.getItem("functions") || "[]"
  );

  let isMatched = false;
  for (let j = 0; j < expressions.length; j++) {
    let result = parseExpression(expressions[j]); // Parse the expression
    for (let i = 0; i < functionsFromStorage.length; i++) {
      ...
      if (functionType && Array.isArray(functionType.expression)) {
        ...
        const isEqual =
          JSON.stringify(expressions[j]) === JSON.stringify(storedExpression);
        ...
        if (isEqual) {
          ...
          data.push(
            <General
              expression={result}
              viewBox={viewBox}
              storedExpression={JSON.stringify(storedExpression)}
            />
          );
